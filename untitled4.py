...
# Fungsi bantu format label kolom
...
if page == "Informasi":
    ...
    for col in df.select_dtypes(include=['int64', 'float64']).columns:
        if col != 'Personality':
            ...
            ax.set_title(f"Distribusi {format_label(col)} berdasarkan Personality")
...
# -------------------- Halaman Pemodelan --------------------
    ...
    if st.button("\U0001F680 Latih Model"):
        ...
        st.subheader("\U0001F9E9 Confusion Matrix")
        ...
        st.subheader("\U0001F4CC Pentingnya Fitur")
        importances = model.feature_importances_
        imp_df = pd.DataFrame({'Fitur': X.columns, 'Pentingnya': importances})
        imp_df['Fitur'] = imp_df['Fitur'].apply(format_label)
        imp_df = imp_df.sort_values(by='Pentingnya', ascending=False)
...
# -------------------- Halaman Prediksi --------------------
    ...
    if st.session_state.model is None:
        ...
        for col in df.columns:
            if col != 'Personality':
                ...
                if df[col].dtype in [np.float64, np.int64]:
                    val = st.number_input(f"{label}", float(df[col].min()), float(df[col].max()), float(df[col].mean()))
                else:
                    val = st.selectbox(f"{label}", sorted(df[col].dropna().unique()))
                input_data[col] = val
        ...
        if st.button("Prediksi"):
            ...
            st.subheader("\U0001F4C8 Probabilitas Prediksi")
